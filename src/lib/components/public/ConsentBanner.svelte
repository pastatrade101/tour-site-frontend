  ...
  >
    <div class="flex min-w-0 flex-col gap-2.5 sm:flex-row sm:items-start sm:justify-between">
      <div class="flex min-w-0 items-start gap-3">
        ...
        <p class="min-w-0 break-words text-[13px] leading-5 text-ink/75 sm:text-sm sm:leading-6">{$t('ui.we_use_analytics_to_improve')}<a class="font-semibold text-forest underline dark:text-goldfinch-gold" href="/privacy">{$t('ui.privacy')}</a>.
        </p>
      </div>
