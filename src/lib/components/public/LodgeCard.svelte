...
<a href={`/accommodation/${lodge.slug}`} class={`group relative flex h-full flex-col overflow-hidden rounded-xl border border-ink/10 bg-surface shadow-[0_12px_34px_rgba(57,61,50,0.07)] transition duration-300 hover:-translate-y-1 hover:border-goldfinch-gold/40 hover:shadow-[0_24px_56px_rgba(57,61,50,0.15)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-goldfinch-gold ${feature ? 'md:grid md:grid-cols-2' : ''}`}>
  <div class={`relative overflow-hidden bg-deep-green ${feature ? 'aspect-[4/3] md:aspect-auto md:min-h-[430px]' : 'aspect-[16/11]'}`}>
    {#if imageUrl}
      <Img
        record={lodge}
  ...
  <div class={`flex flex-1 flex-col ${feature ? 'justify-center p-7 md:p-10' : 'p-4 md:p-5'}`}>
    ...
    <h3 class={`mt-2.5 font-serif font-semibold leading-[1.15] text-heading ${feature ? 'text-3xl md:text-[38px]' : 'min-h-[2.3em] text-xl'}`}>{lodge.name}</h3>
    {#if summary}
      <p class={`mt-2.5 text-[13px] leading-6 text-ink/65 ${feature ? 'md:text-base md:leading-8' : 'line-clamp-2 min-h-12'}`}>{summary}</p>
    {:else if !feature}
      <p class="mt-2.5 min-h-12 text-[13px] leading-6 text-ink/50">A carefully selected stay that can be included in a private Goldfinch itinerary.</p>
    {/if}

    <div class="mt-3 flex min-h-5 flex-wrap items-center gap-x-2.5 gap-y-1 text-[10px] font-semibold text-forest/75">
    ...
    <div class="mt-3.5 flex min-h-10 items-center justify-between gap-3 border-t border-ink/10 pt-3">
