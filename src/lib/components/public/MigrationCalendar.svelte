    ...
    <div class="container-shell">
      ...
      <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" use:staggeredCardReveal={{ y: 18, stagger: 0.06 }}>
        {#each entries as entry}
          ...
          <article
            class={`group relative flex flex-col overflow-hidden rounded-[12px] border bg-surface shadow-[0_14px_44px_rgba(57,61,50,0.06)] transition hover:shadow-[0_20px_50px_rgba(57,61,50,0.10)] ${current ? 'border-goldfinch-gold ring-2 ring-goldfinch-gold/40' : 'border-ink/10 hover:border-forest/25'}`}
          >
            {#if entry.image_url}
              <div class="relative aspect-[16/10] overflow-hidden">
                <img
                  ...
                  decoding="async"
                />
                <span class="absolute left-3 top-3 rounded-full bg-heading/85 px-3 py-1 text-xs font-bold uppercase tracking-wide text-white backdrop-blur">{entry.month}</span>
                {#if current}
                  <span class="absolute right-3 top-3 rounded-full bg-goldfinch-gold px-2.5 py-1 text-[11px] font-bold text-heading">This month</span>
                {/if}
              </div>
            {/if}
            ...
            <div class="flex flex-1 flex-col gap-2 p-5">
              {#if !entry.image_url}
                <div class="flex items-center justify-between gap-2">
                  ...
                  {#if current}<span class="rounded-full bg-goldfinch-gold px-2.5 py-1 text-[11px] font-bold text-heading">This month</span>{/if}
                </div>
              {/if}
              {#if entry.location}
