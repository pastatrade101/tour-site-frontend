      ...
      {:else}
        <div class="grid gap-3">
          {#each sortedInclusions as item (item.id)}
            <article class="flex items-center gap-4 rounded-[8px] border border-ink/10 bg-surface p-4 shadow-[0_10px_30px_rgba(57,61,50,0.05)]" transition:fade={{ duration: 120 }}>
...
{#if modalOpen}
  <div class="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4 backdrop-blur-sm" transition:fade={{ duration: 140 }}>
    <form
      class="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[10px] border border-ink/10 bg-surface p-5 shadow-[0_24px_80px_rgba(57,61,50,0.18)] sm:p-6"
